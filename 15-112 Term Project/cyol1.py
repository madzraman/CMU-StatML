#CREATE YOUR OWN LEVEL 6X6 BOARD
from random import *
import copy
from tkinter import *
...
class VerticalPiece(object):
    def __init__(self, row, col, length):
        ...
        if (self.length == 3) and ((self.currRow == 4) or (self.currRow == 5)):
                self.currRow = 3
        elif (self.length == 2) and (self.currRow == 5):
            self.currRow = 4
...
class HorizontalPiece(object):
    def __init__(self, row, col, length):
        self.length = length
        self.currRow, self.currCol = row, col #left or top most square      
        if (self.length == 3) and ((self.currCol == 4) or (self.currCol == 5)):
                self.currCol = 3
        elif (self.length == 2) and (self.currCol == 5):
            self.currCol = 4 #to make sure pieces only spawn inside the board
        self.color = 'purple'
    def __eq__(self, other):
...
class TargetPiece(HorizontalPiece):
    def __init__(self, row, col, length):
        ...
        if (self.length == 3) and ((self.currCol == 4) or (self.currCol == 5)):
            ...
            self.currCol = 4 #to make sure pieces are only inside board        
        self.color = 'red'
def init(data):
    data.cmode = 'create'
    font = data.height//8
    ...
    data.undone = []
    data.gridNum,data.font  = 6, data.height//8
...
def createMousePressed(event, data):
    ...
    if (data.boardX+4.5*data.gridSize < event.x < data.boardX+data.boardSize+data.gridSize//2)\
    and (data.boardY+data.boardSize+data.gridSize//3 < event.y < data.boardY+data.gridSize*7.5):
        data.cmode = 'play'
...
def createRedrawAll(canvas, data):
    ...
    for i in range(data.gridNum):
        for j in range(data.gridNum):
            canvas.create_rectangle(data.boardX + j*data.gridSize, \
                ...
                data.gridSize, data.boardY + (i+1)*data.boardY, fill = '',\
    ...
    canvas.create_text(data.boardX+data.boardSize-data.gridSize//2, data.boardY+data.boardSize+data.gridSize,\
