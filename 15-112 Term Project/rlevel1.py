...
def goodBoard(data):
    if (len(data.pieces) < 5) or (len(data.pieces)> 10) or boardIsWinnable(data)\
     or (twoVertsBlockTarget(data)==False) or (filledRowOrCol(data)==True):
        return False
    return True

def twoVertsBlockTarget(data):
    count = 1
    for piece in data.pieces:
        if isinstance(piece, VerticalPiece) and \
        ((piece.currRow == 1) or (piece.currRow + 1 == 1)):
            count += 1
        if count >=3: #we know board is complex enough
            return True
    return False
...
def createRandomBoard(data):
    ...
    data.pieces.append(data.target)
