...
def goodBoard(data):
    if (len(data.pieces) < 20) or (len(data.pieces)> 29) or boardIsWinnable(data)\
     or (vertsBlockTarget(data)==False) or (filledRowOrCol(data)==True):
        return False
    return True

def vertsBlockTarget(data):
    count = 1
    for piece in data.pieces:
        if isinstance(piece, VerticalPiece) and \
        ((piece.currRow == 1) or (piece.currRow + 1 == 1)):
            count += 1
        if count >=5: #we know board is complex enough
            return True
    return False
