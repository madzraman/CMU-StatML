#CREATE YOUR OWN LEVEL 12x12 BOARD
from random import *
import copy
from tkinter import *
...
class VerticalPiece(object):
    def __init__(self, row, col, length):
        ...
        if (self.length == 3) and ((self.currRow == 10) or (self.currRow == 11)):
                self.currRow = 9
        elif (self.length == 2) and (self.currRow == 11):
            self.currRow = 10
...
class HorizontalPiece(object):
    def __init__(self, row, col, length):
        self.length = length
        self.currRow, self.currCol = row, col #left or top most square      
        if (self.length == 3) and ((self.currCol == 10) or (self.currCol == 11)):
                self.currCol = 3
        elif (self.length == 2) and (self.currCol == 11):
            self.currCol = 10 #to make sure pieces only spawn inside the board
        self.color = 'purple'
    def __eq__(self, other):
...
class TargetPiece(HorizontalPiece):
    def __init__(self, row, col, length):
        ...
        if (self.length == 3) and ((self.currCol == 10) or (self.currCol == 11)):
            ...
            self.currCol = 10 #to make sure pieces are only inside board        
        self.color = 'red'
def init(data):
    data.c2mode = 'create'
    font = data.height//8
    ...
    data.undone = []
    data.gridNum,data.font  = 12, data.height//8
...
def createMousePressed(event, data):
    ...
    if (data.boardX+9.5*data.gridSize < event.x < data.boardY+data.boardSize+data.gridSize//3)\
    and (data.boardX+data.boardSize+data.gridSize < event.y < data.boardY+data.gridSize*15):
        data.c2mode = 'play'
...
def createRedrawAll(canvas, data):
    ...
    canvas.create_text(data.boardX+data.boardSize-data.gridSize, data.boardY+data.boardSize+data.gridSize*2,\
